...
## 8. Rate Limiting — **не реализовано**

Задача `A-10` в TASKS.md помечена как блокирующая: `POST /auth/register` и
`POST .../public-bookings` открыты миру без ограничений. Это надо закрыть до
публичного запуска.
