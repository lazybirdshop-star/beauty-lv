...
/**
 * До восьми производных оттенков земли текущего мира.
 *
 ...
 * же механизмом.
 *
 * Повторы убираются — по той же причине и тем же способом, что в `inkRamp`.
 * Подтягивание сходится: у светлой земли все шаги вверх упираются в один и
 * тот же предел, и восемь сдвигов дают, скажем, пять разных цветов. Мастеру
 * это показывали как восемь образцов, из которых несколько неотличимы, и
 * выбор любого из них подсвечивал их все разом — образцы сравниваются по
 * значению. React при этом честно ругался на повторяющиеся ключи.
 */
export function backgroundRamp(colors: ThemeColors): string[] {
  const shades = [-9, -6, -3, 0, 3, 6, 9, 13].map((offset) =>
