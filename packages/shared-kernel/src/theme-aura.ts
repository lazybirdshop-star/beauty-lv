 ...
 * в `page-design.ts`).
 *
 * **Пастель файла сохранена дословно — она не была проблемой.**
 *
 * Первая редакция этого модуля углубила розу и лиловый до 4.5:1 против
 * земли, и это была ошибка измерения: норма меряет пару «текст ↔ фон», а
 * роза и лиловый в этом мире — **заливка**, а не текст. Чернь мира на них
 * даёт 5.98:1 и 6.10:1, то есть пастель файла проходит норму с запасом.
 * Единственная действительно проваленная пара оригинала — **белый** текст
 * на розе (2.19:1) в `.cta-btn.ready`; вместо того чтобы заменить белый на
 * чернь, я перекрасил саму пастель и увёл мир от его облика.
 *
 * Поэтому здесь: заливки — точные значения файла, а текст на них —
 * `accentContrast`, равный черни. Роль объявлена данными
 * (`world.accentRole: 'fill'`), и `theme.test.ts` меряет по роли: для мира
 * заливок проверяется чернь **на** акценте, а не акцент на земле.
 *
 * Что действительно исправлено, потому что несёт текст: приглушённый
 * `#9A938C` давал 2.72:1 на перламутре и набирал им подписи в 11–13px.
 * Тон и насыщенность сохранены (30°), опущена светлота до проходящей
 * ступени.
 *
 * Градиент **буквами** (имя мастера, час ближайшего окна) — третий случай,
 * и файл решает его сам: свой цветной текст он красит глубже пастели
 * (`.d.today` — `#9A86D6`). Резолвер выводит эту пару из заливки
 * автокоррекцией до 3:1 — порога WCAG для дисплейного кегля, — и приходит
 * почти ровно к авторскому значению.
 *
 ...
 * чернь на розе 5.98:1 и на лиловом 6.10:1 — это и есть пара заливки;
 * край 3.47:1.
 */
export const AURA_THEME_PRESETS: Record<AuraThemePresetKey, ThemePreset> = {
  'aura-pearl': {
    ...
    colors: {
      ...
      inkSoft: '#5A544F',
      inkFaint: '#726B64',
      /* Роза файла, без единой правки: в этом мире она заливка, и текст на
         ней — чернь мира, а не белый. */
      accent: '#D9A0AE',
      accentContrast: '#33302E',
      accentSoft: '#F7EEF0',
    },
  },
};
 ...
 */
export const AURA_ACCENT_TO = '#B9A8E3';
...
export const AURA_DESIGN_PRESET: DesignPreset = {
  ...
  world: {
    accentTo: AURA_ACCENT_TO,
    surfaceTint: AURA_SURFACE_TINT,
    /* Акцент этого мира — заливка, а не текст: норму проходит чернь на нём,
       а не он на земле. Роль объявлена данными, потому что от неё зависит,
       что именно обязан измерить тест. */
    accentRole: 'fill',
  },
};
