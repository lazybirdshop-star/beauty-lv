...
| bio                                  | text nullable     |                                                                    |
| avatar_url                           | text nullable     | Лицо человека на публичной странице (миграция 0047)                |
| avatar_focal                         | jsonb nullable    | Точка кадрирования этого снимка; `null` = центр                    |
| status                               | enum              | `active`, `invited`, `disabled`                                    |
| created_at / updated_at / deleted_at |                   |                                                                    |

**Портрет принадлежит человеку, а не оформлению страницы.** До миграции 0047 он
жил в дизайне (`organizations.page_design → masterPhoto.media`) и дублировался в
`organizations.logo_url`. Для страницы одного мастера разницы не было; салонная
модель её создаёт — мастеров восемь, а слот под фото в дизайне один
([SALON.md](SALON.md) §4.3, SL-6). Бэкфилл миграции перенёс снимок владельцу:
сначала из опубликованного дизайна, иначе из `logo_url`.

Следствие принято осознанно: снимок **не участвует** в цикле «черновик →
опубликовать → откатить». Смена лица применяется сразу, и откат версии
оформления её не отменяет — версия восстанавливает оформление, а не сотрудника.
Макету осталось решение «показывать ли портрет» (`masterPhoto.shown`).

После этой же миграции `organizations.logo_url` означает **только знак
заведения**: публикация страницы его не перезаписывает, правит его форма
профиля, показывает кабинет клиента рядом с записью.

### 3.4.1. `organization_invites`

Приглашение сотрудника в салон (миграция 0053, [SALON.md](SALON.md) SL-3).

| Поле               | Тип                  | Описание                                               |
| ------------------ | -------------------- | ------------------------------------------------------ |
| id                 | uuid, PK             |                                                        |
| organization_id    | uuid, FK             |                                                        |
| email              | text                 | Приведён к канону вызывающим                           |
| role               | enum                 | `admin` или `master`; `owner` не приглашают            |
| display_name       | text nullable        | Как звать человека, пока он не вошёл и не назвался сам |
| token_hash         | text unique          | SHA-256; сам токен живёт только в письме               |
| invited_by_user_id | uuid, FK             |                                                        |
| expires_at         | timestamptz          | Семь дней                                              |
| accepted_at        | timestamptz nullable |                                                        |
| accepted_member_id | uuid, FK nullable    | Строка членства, которой приглашение обернулось        |
| revoked_at         | timestamptz nullable | Отзыв — дата, а не удаление строки                     |
| created_at         | timestamptz          |                                                        |

Уникальный **частичный** индекс `(organization_id, email) WHERE accepted_at IS
NULL AND revoked_at IS NULL`: одно живое приглашение на адрес, но пригласить
того же человека второй раз — законное действие, и отозванное месяц назад
приглашение не должно этому мешать.

**Почему не строка в `user_tokens`.** У той `user_id NOT NULL` — она всегда про
существующий аккаунт, а сотрудницу зовут по адресу почты, которого в продукте
может не быть вовсе. Заводить аккаунт заранее нельзя: половина приглашений не
принимается никогда, и база покрылась бы призраками, неотличимыми от живых
людей. Плюс приглашение несёт роль и организацию — две колонки, пустые у всех
прочих назначений, а `user_tokens` одна ровно потому, что у её строк общая
форма.
...
Какие мастера оказывают какие услуги, с возможным переопределением
цены/длительности. Реализовано миграцией 0054 ([SALON.md](SALON.md) §4.4, §4.5,
SL-5).
...
| service_id                | uuid, FK         |                                 |
| duration_override_minutes | integer nullable | `null` — длительность из прайса |
| price_override_amount     | integer nullable | `null` — цена из прайса         |
| created_at / updated_at   |                  |                                 |

Уникальность: (`organization_member_id`, `service_id`). Второй индекс по
`service_id` — под вопрос «кто оказывает эту услугу».

`organization_id` здесь **нет** намеренно: обе ссылки уже несут организацию, и
третья колонка с тем же значением — третье место, где оно однажды разойдётся.
Что пара принадлежит одной организации, проверяет тот, кто пишет.

**Отсутствие строки означает «не оказывает», а не «оказывает по прайсу».**
Обратное прочтение вернуло бы ровно ту дыру, ради которой таблица заведена: до
неё гость мог выбрать окно барбера и услугу «наращивание ресниц».

**Пустое переопределение — не ноль.** Ноль это «бесплатно», и подменять им «как
у всех» значит однажды продать работу даром; отсюда `nullable` вместо
копирования прайса в каждую строку — иначе изменение цены услуги пришлось бы
разносить по всем мастерам, и забытая строка тихо продавала бы по прошлогодней.

**Засев обязателен и идёт той же миграцией** — декартово произведение «каждый
живой участник × каждая услуга его организации». Без него организация без строк
перестала бы принимать записи в момент, когда бронирование начало проверять
исполнителя. Дальше умолчание держится с двух концов: новая услуга достаётся
всем, кто работает, а пришедший в салон получает весь прайс.
...
Заблокированное время — «меня здесь нет» (спецификация дашборда §24): обед,
личное, отпуск. Миграция `0055_time-blocks`.

| Поле                    | Тип                | Описание                                          |
| ----------------------- | ------------------ | ------------------------------------------------- |
| id                      | uuid, PK           |                                                   |
| organization_id         | uuid, FK           | Организация — граница арендатора для чтения       |
| organization_member_id  | uuid, FK           | Чьё это время                                     |
| starts_at               | timestamptz        | Начало                                            |
| ends_at                 | timestamptz        | Конец; `CHECK (ends_at > starts_at)`              |
| title                   | text, nullable     | Причина словами; пусто — «Нет приёма»             |
| created_by_user_id      | uuid, FK, nullable | Кто поставил — владелица может ставить за мастера |
| created_at / updated_at | timestamptz        |                                                   |

Индексы: (`organization_member_id`, `starts_at`) — проверка «не в блоке ли» при
публикации окна и записи; (`organization_id`, `starts_at`) — командный
календарь.

**Отрезок, а не окна.** Блок не превращается в снятые окна: окна под ним
снимаются при постановке, но сам блок остаётся строкой и дальше запрещает
открывать это время — `POST /slots`, `POST /slots/bulk`, ручная запись и
перенос проверяют пересечение с блоком внутри своей транзакции. Блок может
идти сквозь сутки: отпуск на неделю — одна строка.

**Повтор — конкретными строками, а не правилом** (в духе [SALON.md](SALON.md)
§6.2): «обед каждый четверг до 8 октября» — N строк, посчитанных по
гражданским дням пояса заведения, чтобы 13:00 оставалось 13:00 после перевода
часов. Правило повторения с исключениями — отдельная модель, и без неё
отменить один четверг значит удалить одну строку.

**Поверх визита не ставится.** Одиночный блок, пересекающий активный визит,
отклоняется; у повтора такой день пропускается и называется в ответе. Молча
накрыть записанного клиента блоком значило бы потерять его из вида.
...
### 3.19. `staff_compensation` и `payouts`

Проценты и ведомость салона ([SALON.md](SALON.md) §7.2–§7.3, SL-11/12),
миграция `0058`.

**`staff_compensation`** — условия расчёта с мастером.

| Поле                   | Тип                | Описание                                       |
| ---------------------- | ------------------ | ---------------------------------------------- |
| id                     | uuid, PK           |                                                |
| organization_id        | uuid, FK           | Граница арендатора                             |
| organization_member_id | uuid, FK           | Чьи условия                                    |
| type                   | enum               | `percent`, `chair_rent`, `salary_plus_percent` |
| percent_bps            | integer, nullable  | Базисные пункты: 4500 = 45 %                   |
| rent_amount            | integer, nullable  | Аренда кресла в центах за `rent_period`        |
| rent_period            | enum, nullable     | `day`, `week`, `month`                         |
| salary_amount          | integer, nullable  | Оклад в центах **в месяц**                     |
| currency               | text               |                                                |
| effective_from         | date               | С какого гражданского дня действуют            |
| created_by_user_id     | uuid, FK, nullable |                                                |
| created_at             | timestamptz        |                                                |

Строки **только добавляются**: новые условия действуют с `effective_from`, конец
прежних выводится из следующей строки, а не хранится (поля `effective_to` из
проекта в схеме нет — его пришлось бы обновлять). Две строки с одной датой —
действует выпущенная позже. Форма строки закреплена CHECK-ограничением: у
процента только `percent_bps`, у аренды только сумма и период, у оклада — оклад
и процент.

**Оклад — в месяц, а не «за период ведомости».** Сумма «за период» теряет
смысл, как только период не ровный месяц; месячный оклад делится на дни по
календарю, как и месячная аренда.
...
| ---------------------------------------- | --------------------- | ------------------------------------------------- |
| id                                       | uuid, PK              |                                                   |
| organization_id / organization_member_id | uuid, FK              |                                                   |
| period_start / period_end                | date                  | Гражданские дни, обе границы включительно         |
| currency                                 | text                  |                                                   |
| revenue_amount / bookings_count          | integer               | Доход и число завершённых визитов за период       |
| master_amount                            | integer               | К выплате; у аренды в убыточный период — меньше 0 |
| salon_amount                             | integer               | Остаётся салону; `master + salon = revenue`       |
| breakdown                                | jsonb                 | Отрезки периода с условиями, по которым посчитано |
| status                                   | enum                  | `draft`, `approved`, `paid`                       |
| created_by / approved_by / paid_by       | uuid, FK, nullable    |                                                   |
| approved_at / paid_at                    | timestamptz, nullable |                                                   |
| created_at / updated_at                  | timestamptz           |                                                   |

Уникальность: `(organization_member_id, period_start, period_end)`. Суммы —
снимки: черновик пересчитывается (прежние черновики, задевающие период,
снимаются в той же транзакции), утверждённая и выплаченная ведомость не
меняются, и пересчёт пропускает мастера, у которого за период такая уже есть.
Доход считается по завершённым визитам, день визита — дата в поясе заведения.
