...
export const bookings = pgTable(
  ...
  (table) => [
     ...
     * платформе. Индекс частичный, потому что и вопрос частичный: ждущих
     * заявок единицы ровно потому, что этот проход их и гасит, и держать в
     * индексе остальные сотни тысяч не за чем.
     */
    index('bookings_pending_slot_idx')
      .on(table.publishedSlotId)
