 ...
 * запись. Кто он, кто ещё должен узнать, на каком языке каждый читает и в
 * каком поясе живёт салон, выясняют уведомления: иначе модуль записей
 * пришлось бы научить читать пользователей и организации ради того, что его
 * не касается.
 */
...
/**
 * Уведомления о событиях календаря — мастеру визита и администрации салона.
 *
...
export class BookingPushService {
  ...
  /**
   * Клиент отменил визит сам. Мастер и стойка узнают об этом так же немедленно,
   * как о новой записи: освободившийся час продаётся, только пока он не прошёл.
   */
  ...
  /**
   * Клиент перенёс визит сам — по той же причине: старый час освободился и
   * продаётся, только пока не прошёл, а новый — время, о котором ещё не знают.
   */
  ...
  private async notify(
    ...
    try {
      ...
      const audience = await this.recipients.findForBookingEvent(input.organizationMemberId);
      if (!audience) {
      ...
      const results = await Promise.all(
        audience.recipients.map(async (recipient) => {
          ...
          const message = compose(resolveNotificationLocale(recipient.locale), {
            ...
            masterName: recipient.isVisitMaster ? undefined : audience.masterName,
          });
