...
export class ClientBookingsRepository {
   ...
   *
   * Для того, кто уже вошёл. Токен доказывает, что человек держит ссылку на
   * запись, но **не** доказывает, что запись его: ссылка пересылается в чат,
   * остаётся в общем почтовом ящике и живёт в `localStorage` общего планшета
   * 120 дней. Поэтому одного токена мало.
   *
   * Замков здесь три, и каждый закрывает свой случай:
   *
   * 1. `client_user_id IS NULL` — запись, уже привязанную к чужому аккаунту,
   *    не перехватить.
   * 2. Совпадение почты — если у записи есть `guest_email`, забрать её может
   *    только аккаунт с этим адресом. Без этого держатель чужой ссылки
   *    привязывал бы визит к себе и получал в `/client/profile` имя и телефон
   *    настоящего клиента, которых сам токен не открывает.
   * 3. Почта записи проставляется при привязке (`coalesce`) — ровно как в
   *    `linkToClient`, и по той же причине: со следующего раза замок №2 у неё
   *    уже есть.
   *
   * Запись **без** почты остаётся привязываемой по одному токену — иначе
   * гость, записавшийся без адреса (форма его не требует), не увидел бы свой
   * визит никогда. Это осознанная цена необязательной почты, а не недосмотр.
   */
  async claimByPublicToken(
    ...
    const [claimed] = await this.db
      ...
      .set({
        clientUserId: userId,
        guestEmail: sql`coalesce(${bookings.guestEmail}, ${email})`,
        updatedAt: new Date(),
      })
      .where(
        and(
          ...
          isNull(bookings.clientUserId),
          isNull(bookings.deletedAt),
          sql`(${bookings.guestEmail} is null or lower(${bookings.guestEmail}) = ${email})`,
        ),
      )
      .returning({ id: bookings.id });
    ...
    /* Ноль строк означает четыре разных вещи, и экрану они не одно и то же:
       запись уже своя (говорить не о чем), запись чужая, запись записана на
       другую почту или ссылки не существует. Последние три отвечают наружу
       одинаково — иначе по ответу отличали бы живую чужую запись от
       несуществующей и подбирали бы к ней адрес. */
