...
export class JobsRepository {
  ...
  /**
   * Убрать выполненные задачи старше срока хранения.
   *
   * Выполненные строки копились навсегда и намеренно — «письмо ушло — тоже
   * ответ», и на вопрос «а дошло ли до клиента подтверждение» отвечает именно
   * эта таблица. Но «навсегда» и «пока это кому-то нужно» — разные сроки:
   * четыре письма на запись при полусотне записей в день это ~70 000 строк за
   * год, и они остаются в бэкапах, в вакууме и на диске, отвечая на вопрос,
   * который давно никто не задаёт.
   *
   * Удаляются только `done`. Неудачные (`failed`) не трогаются никогда: это
   * работа, которая не сделана, и стереть её значит стереть сам факт, что
   * письмо не дошло.
   */
