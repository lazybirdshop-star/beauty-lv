...
async function main() {
  ...
  try {
    ...
    for (const [viewportName, viewport] of Object.entries(VIEWPORTS)) {
      ...
      /*
       * Полоса уведомления о хранении снимается на время замера.
       *
       * Она прибита к низу окна и стоит поверх страницы, а `scrollIntoView`
       * приводит цель в ближайшее место — то есть нередко под эту полосу.
       * Снимок элемента берёт то, что нарисовано в его рамке, и мерил в итоге
       * не фотографию под текстом, а тёмную плашку уведомления с кнопкой
       * «Понятно»: и «подпись у кнопки героя», и «абзац ночи» на десктопе
       * стабильно проваливались с 2.56:1 и 1.03:1, хотя на самом деле в этих
       * местах 15:1. Проверено дампом (`LANDING_CONTRAST_DUMP`) — в кадре
       * стояла полоса.
       *
       * Скрипт меряет контраст текста над фотографией; полоса согласия к
       * этому вопросу отношения не имеет и в кадре быть не должна.
       */
      await page.addStyleTag({ content: '.storage-notice { display: none !important; }' });
