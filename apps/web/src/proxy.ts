...
export async function proxy(request: NextRequest) {
  /**
   * Единственные публичные адреса внутри охраняемой зоны. Манифест браузер
   ...
   *
   * Проверка по имени файла, а не по конкретному разделу: правило было
   * написано под кабинет мастера, и появившаяся позже панель под него не
   * попала — её манифест уходил в редирект, а иконка панели открывала не
   * очередь заявок. Любой следующий раздел со своим манифестом теперь
   * получает это исключение сам.
   */
  if (request.nextUrl.pathname.endsWith('/manifest.webmanifest')) {
