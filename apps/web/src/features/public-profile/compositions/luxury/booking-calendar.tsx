 ...
 * чернильными швами.
 *
 * Первая полоса — «ближайшее окно»: сетка 1:1, слева дата и время антиквой
 * 40px, справа бронзовая плита «Записаться» у нижнего края. Вторая —
 * «Расписание»: заголовок антиквой с капс-меткой месяца, сетка календаря на
 * голом листе за чернильным швом. Ячейки плоские: день с окнами — чернь,
...
export function BookingCalendar({ data, state, actions }: CalendarSectionProps) {
  ...
  return (
    <section aria-labelledby="booking-heading" id="booking">
      ...
      {/* Полоса «ближайшее окно»: дата антиквой слева, бронзовая плита
          справа; полосу закрывает чернильный шов. */}
      {facts.nearestSlot ? (
        <div className="grid grid-cols-2 border-b border-border-strong">
          <div className="border-r border-border-strong px-[18px] py-6">
            <span className={cn('block', CAPTION_CLASS)}>{t.publicPage.nearestWindow}</span>
            <span className="mt-3 block font-display text-[40px] leading-[1.05] tabular-nums [font-weight:var(--display-weight)] text-ink">
              {facts.nearestLabel}
              <br />
              {facts.nearestSlot.time}
            </span>
          </div>
          <div className="flex items-end p-[18px]">
            <button
              type="button"
      ...
      <div className="px-[18px] pb-4 pt-7">
        <div className="flex items-end justify-between gap-3">
          <h3 className="min-w-0 font-display text-[30px] leading-none [font-weight:var(--display-weight)] text-ink">
        ...
        {!slotMonths.has(monthKey(visible.year, visible.month)) ? (
          <p className="mt-4 text-center text-sm text-ink-soft">{t.publicPage.noSlotsThisMonth}</p>
        ) : null}
