...
export function VisitRow({
  ...
  /* Подтверждённая запись бейджа не носит: это состояние по умолчанию, и
     восемьдесят зелёных плашек подряд топили единственное «Ждёт ответа» —
     тот самый статус, ради которого в список и заходят. Остальные статусы
     (ждёт, отменена, не пришёл, завершена) остаются названными. */
  const showStatus = statusVisible ?? (register !== 'team' && status !== 'confirmed');
