...
export function BookingsScreen({ slug, initialFilter }: BookingsScreenProps) {
   ...
   *
   * `enabled: sheetOpen` — потому что читает её только шторка, а открывают её
   * далеко не каждый раз, когда заходят на экран записей. Список приезжает
   * целиком и без предела: у мастера с восемьюстами клиентами это сотни
   * килобайт со всеми полями, включая заметки, — и раньше они уезжали на
   * телефон при каждом открытии экрана, даже если шторку так и не тронули.
   *
   * Ключ общий с экраном клиентов, поэтому у того, кто пришёл оттуда, книга
   * уже в кэше и шторка открывается без ожидания.
   */
