 ...
 *
 * Час — 60 px во всех регистрах, как в прототипе «Кабинет 2026» (`--h`):
 * командный день отличается плотностью подписей в блоке, а не масштабом
 * времени, — иначе час в команде и час в дне были бы разной длины.
 */
export const DENSITY = { spacious: 30, compact: 30, phone: 30 } as const;
...
/** Подряд идущие свободные окна колонки — одним отрезком. */
export interface FreeRun {
  /** Первое окно отрезка: нажатие открывает его. */
  first: FreeSlot;
  from: number;
  to: number;
  hidden: boolean;
...
/**
 * Свободное время колонки отрезками.
 *
 * Пять рамок «09:00, 09:30, 10:00, 10:30, 11:00» столбиком превращали
 * командную сетку в шум: свободное время — один спокойный отрезок
 * «09:00–11:30». Скрытые окна отдельным отрезком: у них другой смысл.
 */
export function freeRuns(free: readonly FreeSlot[], slotMinutes = SLOT_MINUTES): FreeRun[] {
  const runs: FreeRun[] = [];
  for (const slot of [...free].sort((a, b) => a.at - b.at)) {
    const last = runs.at(-1);
    if (last && last.to === slot.at && last.hidden === slot.hidden) {
