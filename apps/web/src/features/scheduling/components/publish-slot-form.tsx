...
/**
 * Сколько времени открывает одно нажатие.
 *
 * Не длина окна — её у окна нет, — а сколько окон подряд завести: полчаса это
 * одно окно, два часа — четыре. Те же значения, что у шага во вкладке
 * «Период», чтобы две дороги к одному не расходились в словах.
 */
...
export function PublishSlotForm({
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    /* Длительность превращается в окна: у окна нет своей длины, она равна
       шагу сетки (`SLOT_MINUTES`), и «два часа» — это четыре окна подряд.
       Так же устроена вкладка «Период», и результат одинаков в обеих. */
