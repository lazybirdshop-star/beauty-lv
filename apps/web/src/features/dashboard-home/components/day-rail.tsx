    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-end gap-x-4 gap-y-1" aria-live="polite">
        <span className="font-display text-[clamp(2rem,7vw,2.75rem)] leading-[0.82] tabular-nums text-ink">
          {String(readoutHour).padStart(2, '0')}:00
        </span>
        <span className="pb-1 text-[13px] text-ink-soft">
          {readout?.detail ?? t.home.railEmpty}
        </span>
      </div>
      ...
      >
        {HOURS.map((hour) => {
          ...
          const shared = cn(
            'action-motion flex h-full w-full items-end',
