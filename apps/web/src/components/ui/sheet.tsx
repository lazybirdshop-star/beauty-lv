 ...
 *
 * В кабинете (`surface="app"`) это лист прототипа «Кабинет 2026»: 520 px в
 * 12 px от правого края, по центру высоты, тень «парит», шапка —
 * прокручиваемое тело — закреплённый подвал с линией; на телефоне — лист
 * снизу с ручкой и жестом «потянуть вниз». В публичных мирах — плавающая
 * карточка у нижнего края, какой она была всегда. Геометрию решает CSS по
 * ширине окна (`dashboard-shell/styles/app.css`), разметка одна.
 *
