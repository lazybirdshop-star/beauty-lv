...
export default async function MarketingHomePage() {
  ...
  return (
    <>
      {/*
       * Модель весит 614 КБ и нужна первому же экрану, но живёт за динамическим
       * импортом три-джея: без предзагрузки браузер узнаёт о ней только после
       * того, как выполнит чанк сцены. React 19 поднимает этот тег в <head>.
       */}
      {/*
       * Низкий приоритет намеренно. Модель весит 614 КБ, и на мобильной сети
       * это несколько секунд полосы; запрошенная наравне со стилями, она
       * отодвигала и их, и фотографию первого экрана — то есть всё, из чего
       * складывается первый кадр. Предзагрузка остаётся, чтобы браузер узнал
       * о модели до того, как выполнится чанк сцены, но забирает полосу
       * последней: устройство приходит в кадр позже текста и по замыслу.
       */}
      <link
        rel="preload"
        as="fetch"
        href="/landing/models/iphone-16-pro.glb"
        crossOrigin=""
        fetchPriority="low"
      />
