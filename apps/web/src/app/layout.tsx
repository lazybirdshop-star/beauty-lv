...
/*
 * Ось оптического размера обязательна, а не украшение.
 *
 * Inter — переменная гарнитура с двумя осями, и вторая, `opsz`, задаёт
 * рисунок под кегль: у текстового начертания шире просветы и мягче углы,
 * у дисплейного — теснее и жёстче. `next/font` по умолчанию берёт только
 * `wght`, и заголовок в 96px набирался текстовым рисунком: строка
 * заявления первого экрана выходила шире на треть слова, а абзац под ней
 * переносился в три строки вместо двух. Присланный макет просит
 * `opsz,wght@14..32,400..700` — эта строка и есть тот же запрос.
 */
const inter = Inter({
  variable: '--font-inter',
  subsets: ['latin', 'cyrillic', 'latin-ext'],
