...
function LoginForm() {
  ...
  return (
    <div className="auth__stack">
      ...
      {/* Путь на регистрацию обязан быть виден отсюда, а не только с
          лендинга: сюда приходят по привычке. Про код приглашения здесь больше
          не спрашивают — кодов нет с миграции 0038, и `/register` это форма
          заявки либо открытая регистрация, смотря по режиму платформы. */}
      <p className="auth__alt">
        {t.auth.noAccountQuestion} <Link href="/register">{t.auth.goToRegister}</Link>
      </p>
    </div>
  );
}
