...
**CTA-03 — MUST** write button labels that answer _what will happen_: verb + outcome — «Забрать расчёт», «Записаться на замер», «Посмотреть меню». **MUST NOT** ship «Отправить», «Подробнее», «Узнать больше», "Submit", "Learn more".
...
.is-in .rise {
  ...
  transition:
    ...
    filter 900ms cubic-bezier(0.22, 1, 0.36, 1) var(--d, 0ms);
}
```
