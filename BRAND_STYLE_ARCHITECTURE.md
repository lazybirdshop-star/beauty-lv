...
export const BRAND_STYLE_KEYS = [
  ...
  'organic',
  'neo-glass',
] as const; // семь композиций — утверждено окончательно (§14.2)
...
алиаса стоит шаг, который его снимает (M3–M7), и конечное состояние
реестра известно заранее: семь собственных композиций. Постоянный шеринг
существует ровно один — `soft-studio → soft` — и живёт он не здесь, а в
резолвере ключей (§8.1): это одна школа на уровне дизайна, а не заём
чужого дерева.
...
**M5. Neo Glass composition.** Новый мир по BRAND_STYLES.md §9 и
`docs/references/neo-glass/`: пространственная композиция, слоистость,
spring+blur хореография, squircle-геометрия. До включения в реестре —
алиас на soft (страницы neo-glass работают как сейчас). _Выход: мир принят
по референсам и монохром-тесту (§15)._
...
Композиций — **семь**: `soft`, `poster`, `editorial`, `minimal`, `luxury`,
`organic`, `neo-glass` (§8.1). `soft-studio`→`soft`, `poster`→`poster`;
`editorial` получает собственную журнальную композицию по своей спеке
(BRAND_STYLES.md §5) — алиас `editorial → poster` отменён. Единственный
допустимый шеринг композиции — `soft-studio → soft`: это одна школа,
различие только в палитре и шрифтах. Токенные классики (`blush-rose` и
...
4. **Монохром-тест:** скриншоты всех миров в grayscale без изображений
   различимы по композиции (проверка на M-шагах M3–M7; снимки — тем же
   харнессом §16).
5. **Референс-тест:** каждый мир, построенный с нуля (minimal, luxury,
   editorial, organic, neo-glass), соответствует своим референсам
