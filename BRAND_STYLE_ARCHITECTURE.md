...
version: 1.3
description: Архитектура «один движок — пять визуальных композиций»: soft, poster, luxury, aura, funk; разделение shared product logic и style-specific visual composition для публичной страницы мастера
status: Реализовано. Каталог закрыт на пяти мирах.
---
...
Цель: **ONE PRODUCT · ONE BUSINESS LOGIC · FIVE DISTINCT VISUAL WORLDS.**
...
**Действующее состояние.** Каждый из пяти миров каталога держит полный
...
Каталог — **пять миров**: две классики (`soft`, `poster`), `luxury` и два
авторских мира, `aura` и `funk`. Столько же и композиций (§8.1): ключ стиля
равен ключу композиции, шеринга не осталось ни постоянного, ни переходного.
